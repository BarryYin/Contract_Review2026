"""
合规审查引擎：调用 LLM 进行合同风险分析。
T-RO-01: 输出三要素格式（风险描述 + 法律依据 + 修改示例）
增强：语义级风险识别 - 模糊措辞、权利义务不对等、跨条款连锁风险
"""
    ...
    "issues": [
        {
            ...
            "risk_category": "vague_wording / rights_imbalance / cross_clause_conflict / compliance",
            "risk_description": "用通俗易懂的非法律专业语言说明该条款存在什么风险、如果签署可能导致什么后果（50-200字）",
            "legal_basis": "引用具体法条，格式如：依据《民法典》第577条。必须准确引用真实法条，禁止编造不存在的法条",
            "modification_example": {
                "original": "合同中的原文（完整摘录有问题的部分）",
                "suggested": "建议修改后的文本（给出具体可替换的修改版本）"
            },
            "cross_clause_refs": ["与该问题存在逻辑冲突的其他条款引用，如'第八条 保密条款'"]
        }
    ]
}

============================================================
【审查维度】-- 你必须从以下三个维度逐一审查，不得遗漏：
============================================================

维度一：措辞模糊风险（risk_category = "vague_wording"）
----------------------------------------------------------
识别合同中缺乏定量约束、含义模糊、容易被不同解读的表述。重点检查：
1. 时间模糊：如"合理时间内"、"尽快"、"适时"、"必要时"、"视情况"——必须标注，建议替换为具体天数或日期
2. 金额/标准模糊：如"适当补偿"、"合理费用"、"相应赔偿"、"一定数额"——必须标注，建议明确金额范围或计算公式
3. 范围模糊：如"相关资料"、"必要支持"、"其他事项"、"合理范围内"——建议界定具体范围
4. 条件模糊：如"甲方认为必要时"、"乙方应当配合"等无触发标准的条件——建议设定触发条件
5. 质量标准模糊：如"合格"、"满足要求"、"达到标准"等无客观衡量标准的表述——建议引用具体标准或量化指标

对于每个识别到的模糊措辞，modification_example 必须给出量化替换建议。例如：
- original: "甲方应在合理时间内完成交付"
- suggested: "甲方应在收到订单后30个工作日内完成交付"

维度二：权利义务不对等（risk_category = "rights_imbalance"）
----------------------------------------------------------
识别合同中一方权利明显大于另一方、或一方义务明显重于另一方的条款。重点检查：
1. 单方变更权：甲方是否有权"随时变更"、"自行调整"合同条款而乙方无异议权利
2. 单方解除权：一方是否可以无条件或低条件解除合同，而另一方解除条件严苛
3. 免责不对等：一方的免责范围远大于另一方，或仅限制对方追责权利
4. 信息不对称义务：一方被要求提供大量信息/资料，而另一方无对等披露义务
5. 知识产权/成果归属不对等：工作成果全部归一方所有，另一方丧失所有权益
6. 违约责任不对等：对一方的违约处罚远重于对另一方类似违约的处罚
7. 知情权/监督权不对等：一方拥有广泛的检查审计权，另一方无相应权利

维度三：跨条款连锁风险（risk_category = "cross_clause_conflict"）
----------------------------------------------------------
识别不同条款之间的逻辑矛盾、时间冲突、条件冲突。重点检查：
1. 期限冲突：保密期限与合同期限不一致（如保密期短于合同期，或合同终止后保密义务未延续）
2. 条件矛盾：A条款设定的前提条件与B条款的执行条件互相排斥
3. 时间矛盾：付款条件中的验收期限与交付条款中的验收标准在时间上不匹配
4. 责任冲突：违约责任条款中的赔偿范围与免责条款中的免责范围存在交叉或冲突
5. 终止矛盾：终止条款约定的生效时间与通知条款要求的通知期不一致
6. 管辖冲突：争议解决条款同时约定仲裁和诉讼（"或裁或审"无效）

对于跨条款风险，必须填写 cross_clause_refs 字段，列出存在冲突的条款引用。

============================================================
【基础审查要点】（作为补充维度，risk_category = "compliance"）
============================================================
1. 付款条件和违约金是否合理
2. 知识产权归属是否明确
3. 保密条款和竞业限制
...
- legal_basis（法律依据）：必须引用真实存在的具体法条。绝对禁止编造法条编号
...
- risk_category 必须填写，用于区分风险类型
- cross_clause_refs 仅跨条款风险必填，其他类型可留空列表 []
...
async def analyze_contract(raw_text: str, clauses: list[dict] = None) -> dict:
    ...
    Returns:
        ...
        - issues: list of {
            id, title, clause_reference, severity, risk_category,
            risk_description, legal_basis,
            modification_example: {original, suggested},
            cross_clause_refs: list[str]
          }
    """
    if not LLM_API_KEY:
    ...
    for attempt in range(max_retries):
        try:
            ...
            for i, issue in enumerate(result["issues"], 1):
                ...
                issue.setdefault("risk_category", "compliance")
                issue.setdefault("risk_description", "")
                issue.setdefault("legal_basis", "")
                issue.setdefault("cross_clause_refs", [])

                valid_categories = {"vague_wording", "rights_imbalance", "cross_clause_conflict", "compliance"}
                if issue["risk_category"] not in valid_categories:
                    issue["risk_category"] = "compliance"

                if not isinstance(issue.get("cross_clause_refs"), list):
                    issue["cross_clause_refs"] = []

                if not isinstance(issue.get("modification_example"), dict):
                    issue["modification_example"] = {"original": "", "suggested": ""}
                else:
            ...
            for issue in result["issues"]:
                cat = issue.get("risk_category", "compliance")
                category_counts[cat] = category_counts.get(cat, 0) + 1
            logger.info(
                f"LLM analysis complete: score={result['risk_score']}, "
                f"issues={len(result['issues'])}, categories={category_counts}"
            )
            return result
